...
public class Game {
	 ...
	 * 
	 * public void setMoves(Piece piece, int col, int row) { this.moves = piece +
	 * " to " + col + row + ". "; }
	 */
	...
	/*
	 * public void setGameNumber(int gameNum) { this.gameNum = gameNum; }
	 * 
	 * 
	...
	/*
	 * public void newGame() { // 0 = white and 1 = black // create pawns for (int i
	 * = 0; i < 8; i++) { board.setPiece(new Pawn(PieceType.PAWN, new Point(i, 6),
	 * 0)); board.setPiece(new Pawn(PieceType.PAWN, new Point(i, 1), 1)); }
	 * 
	 * // 0 = white and 1 = black // create Bishop, Knights, Rooks for (int i = 0; i
	 * < 2; i++) { board.setPiece(new Bishop(PieceType.BISHOP, new Point((i * 3) +
	 * 2, 0), 1)); board.setPiece(new Bishop(PieceType.BISHOP, new Point((i * 3) +
	 * 2, 7), 0)); board.setPiece(new Knight(PieceType.KNIGHT, new Point((i * 5) +
	 * 1, 0), 1)); board.setPiece(new Knight(PieceType.KNIGHT, new Point((i * 5) +
	 * 1, 7), 0)); board.setPiece(new Rook(PieceType.ROOK, new Point((i * 7), 0),
	 * 1)); board.setPiece(new Rook(PieceType.ROOK, new Point((i * 7), 7), 0)); }
	 * 
	 * // 0 = white and 1 = black // Create Queens board.setPiece(new
	 * Queen(PieceType.QUEEN, new Point(4, 0), 1)); board.setPiece(new
	 * Queen(PieceType.QUEEN, new Point(4, 7), 0));
